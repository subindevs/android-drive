...
package me.proton.core.drive.linkupload.domain.manager
...
class UploadSpeedManager @Inject constructor(
    ...
    private fun checkTime(
        ...
        if (clockElapsedTime >= THRESHOLD) {
            CoreLogger.d(
                TRACKING,
                "checkTime(upload) elapsed(sec) ${clockElapsedTime.toSeconds()}"
            )
            if (stopWatch.isRunning()) {
                ...
                if (stopElapsedTime < MAX_LIMIT) {
                    ...
                    CoreLogger.d(
                        TRACKING,
                        "checkTime(upload) ignoring elapsed(sec): ${stopElapsedTime.toSeconds()} " +
